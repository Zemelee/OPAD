...
class CDDataset(Dataset):
    def __init__(self, dataset, principle="", conv_adapter=None):
        self.dataset = dataset # 打乱后的数据集
        self.principle = principle # 指定原则
        self.conv_adapter = conv_adapter # 提示词"你是有用的ai{}|user{}"
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        pattern = r"(Human|Assistant):" # 按 Human: 或 Assistant: 分割
        dialogue_split = re.split(pattern, dialogue)[1:]  # (human,question,AI,answer)*2=8
        ...
        dialogue_formatted = dialogue_split[:-2] # 去掉最后两个元素
        dialogue_text = self.conv_adapter.format_dialogue("", dialogue_formatted)
        dialogue_text_principle = self.conv_adapter.format_dialogue(
            self.principle, dialogue_formatted
        )
        return {
            "question": dialogue_formatted[-1], # 第二次提问的问题
            "dialogue_text": dialogue_text, # 不带原则的 human,question,AI,answer,human,question
            "dialogue_text_principle": dialogue_text_principle,# 同上+原则(格式化)
            "chosen_answer": answer, # 选择的答案
            "reject_answer": reject_answer, # 拒绝的答案
        }
